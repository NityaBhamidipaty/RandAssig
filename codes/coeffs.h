...
double variance(char *filename){
  double m = mean(filename); //mean of the random variable
  double x, temp =0.0;
  int count = 0;
  FILE* fp = fopen(filename,"r");

  if(fp == NULL){
    printf("variance: Error opening %s\n",filename);
    return -1;
  }
...
//Defining the function for generating other (V = -2ln(1-U)) random numbers
void other(char *str, int len){
  ...
  double x;
  for (i = 0; i < len; i++){
    x = (double)rand()/RAND_MAX; // a uniform random number in (0,1)
    if(x == 1){
      // i--;
      // continue;
      printf("Error\n");
